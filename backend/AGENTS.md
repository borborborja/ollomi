...
This fork deploys `selfhost/` (Compose + `deploy/Dockerfile`), not the upstream cloud backend. Runtime, tests, and dependency rules: `selfhost/AGENTS.md`. The cloud setup below is upstream reference.
...
- **diarizer** (`diarizer/main.py`) — GPU. Speaker embeddings at `/v2/embedding`; match policy in `utils/stt/speaker_match.py`. Called by backend and pusher (`HOSTED_SPEAKER_EMBEDDING_API_URL`).
- **vad** (`modal/main.py`) — GPU. `/v1/vad`. Called by backend only.
...
Coverage matrix: `backend/docs/memory-continuity-gates.md`. CI runs only `python3 backend/scripts/memory-continuity-gauntlet.py --self-check`; live suites record `NOT_RUN` without credentials — never fake `GO`.
