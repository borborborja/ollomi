...
`GET /v1/users/private-cloud-sync` returns the authenticated user's global audio-retention preference and `POST /v1/users/private-cloud-sync?value=<bool>` changes it. New audio jobs snapshot that choice as `retain_audio`, regardless of whether they came from live capture, phone audio, an SD/WAL sync or an import. The default is `false`: after successful processing, Ollomi keeps the conversation and transcript but queues deletion of the source audio. Failed jobs keep the original for retry, and legacy jobs without the field keep their audio. Preference changes never retroactively delete older recordings.

After `last_memory`, `/v4/listen` emits additive `service_status` events. Their public contract is `ready`, `audio_received`, `transcribing`, `no_speech`, `live_stt_unavailable` or `transcription_delayed`, always with `source`; `audio_received` also includes a normalized `audio_level` in the range 0–1. Provider errors are deliberately not exposed. Old clients can ignore these events and continue receiving transcript segment arrays and the final processing event.
