...
El firmware se provisiona en `/data/firmware/` del volumen de audio. `manifest.json` relaciona modelo de dispositivo con `latest` y `stable`; cada entrada contiene `filename`, `sha256`, `version`, `changelog` y los campos OTA de Omi. Solo se sirven binarios instalados localmente; el manifiesto se valida contra SHA-256. No se ha flasheado ningún dispositivo durante la verificación.

El mapa mental usa `/v1/knowledge-graph` del backend self-hosted. Se calcula a partir de las conversaciones y memorias de la cuenta, sus categorías, etiquetas y vínculos de origen, sin Neo4j ni una clave/modelo de IA adicional. Si todavía no hay conversaciones ni memorias, solo aparece el nodo personal; editar o borrar una memoria se refleja en la lectura siguiente. Es un mapa básico de recuerdos, no la extracción semántica avanzada de Omi.

Las vistas previas geográficas usan `/v1/static-map` del backend: por defecto descarga solo las teselas visibles de OpenStreetMap, sin API key, y las conserva en el volumen `/data/map-tiles` durante al menos siete días conforme a su [política de uso](https://operations.osmfoundation.org/policies/tiles/). El backend envía un User-Agent identificable y la imagen muestra «© OpenStreetMap contributors». Si prefiere un servidor de teselas propio, cambie `OLLOMI_MAP_TILE_URL=http://servidor-local/{z}/{x}/{y}.png` en `.env`; el servidor debe devolver PNG de 256 × 256. Sin acceso a las teselas, la app muestra sus puntos sobre un fondo neutro, pero las conversaciones siguen funcionando. Las coordenadas de la zona visible llegan al servidor de teselas elegido; use un servidor propio si no quiere compartirlas con OpenStreetMap. Al tocar la vista previa se abre la aplicación de mapas instalada.
