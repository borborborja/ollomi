...
class Wal {
  final String storageOwner;
  static String get currentStorageOwner {
    final auth = AuthService.instance;
    return '${auth.serverUrl}|${auth.instanceId}|${auth.currentUser?.uid ?? ""}';
  }
  int timerStart;
  BleAudioCodec codec;
  int channel;
  ...
  Wal({
    ...
    this.uploadedAt = 0,
    String? storageOwner,
  }) : storageOwner = storageOwner ?? currentStorageOwner, data = data ?? [] {
  ...
  factory Wal.fromJson(Map<String, dynamic> json) {
    return Wal(
      ...
      storageOwner: json['storage_owner'] ?? 'unassigned',
    );
  }
