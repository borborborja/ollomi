...
class WalFileManager {
  ...
  static Future<List<Wal>> loadWals() async {
    ...
    return Wal.fromJsonList(walsList).where((wal) => wal.storageOwner == Wal.currentStorageOwner).toList();
  }
  ...
  static Future<bool> saveWals(List<Wal> wals) async {
    ...
    final owner = Wal.currentStorageOwner;
    final List<dynamic> preserved = [];
    if (await _walFile!.exists()) {
      final previous = jsonDecode(await _walFile!.readAsString()) as Map<String, dynamic>;
      preserved.addAll((previous['wals'] as List).where((wal) => wal['storage_owner'] != owner));
    }
    final jsonData = {
      'version': 1,
      'timestamp': DateTime.now().millisecondsSinceEpoch,
      'wals': [...preserved, ...wals.where((wal) => wal.storageOwner == owner).map((wal) => wal.toJson())],
    };

    final jsonString = jsonEncode(jsonData);
    final temporary = File('${_walFile!.path}.part');
    await temporary.writeAsString(jsonString, flush: true);
    await temporary.rename(_walFile!.path);
  ...
  static Future<void> clearAll() async {
    await saveWals([]); // Other accounts keep their recordings and recovery metadata.
  }

  static Future<bool> ownsFiles(List<File> files) async {
    final wals = await loadWals();
    final names = wals.expand((wal) => [wal.getFileName(), wal.filePath?.split('/').last]).toSet();
    return files.every((file) => names.contains(file.path.split('/').last));
  }
